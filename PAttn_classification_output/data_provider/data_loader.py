...
class Dataset_page_fault(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='delta_out', scale=True, timeenc=0, freq='h',
                 percent=10, max_len=-1, train_all=False , train_ratio=1.0 , model_id=''):
    ...
    def __read_data__(self):
        ...
        df_raw['pc'] = df_raw['pc'].apply(lambda x: float.fromhex(x))
    ...
    def set_border(self, set_type):
        length = len(self.data_y)
        assert set_type in ['train', 'test', 'val']
        type_map = {'train': 0, 'val': 1, 'test': 2}
        set_type = type_map[set_type]
        ...
        for i in range(len(self.data_x)):
            self.data_x[i] = self.data_x[i][border1:border2]
        self.data_y = self.data_y[border1:border2]
        
        self.data_stamp = data_stamp
        
        self.tot_len = len(self.data_y) - self.seq_len - self.pred_len + 1

        self.period = 1
        self.channel= len(self.data_x)
    ...
    def __getitem__(self, index):
        ...
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        seq_x = [self.data_x[i][s_begin:s_end] for i in range(len(self.data_x))]
        seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
    ...
    def __len__(self):
        return (len(self.data_y) - self.seq_len - self.pred_len + 1) * self.enc_in
